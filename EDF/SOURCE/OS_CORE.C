...
//            OSPrioHighRdy = (INT8U)((OSIntExitY << 3) + OSUnMapTbl[OSRdyTbl[OSIntExitY]]);
			//sam : each preempt task called new schedule again
			EDFPriorityExchange();
        ...
        //         sprintf(s, "switch at %10d",OSTimeGet()/OS_TICKS_PER_SEC);
...
void  OS_Sched (void)
{
	...
	char s[34];
	OS_TCB *currenttemp;
	TASK_EXTRA_DATA *MyPtrss;
    OS_ENTER_CRITICAL();
    if ((OSIntNesting == 0) && (OSLockNesting == 0)) { /* Sched. only if all ISRs done & not locked    */
    // same: called new method

        // call EDF to change the priorty according to earliest deadline
		EDFPriorityExchange();
...
void  EDFPriorityExchange (void){
	INT8U i ;
    char s[80];
	INT8U highestPriority = OS_LOWEST_PRIO; //lowest priority
	INT32S minDeadline =2147483647; //MAX VALUE
    INT8U C=0;
	for (i = 0; i <=  OS_LOWEST_PRIO-1; i++) {//check each priority of task
		if (OSTCBPrioTbl[i] != (OS_TCB *)0 ) {//if the task is not null pointer 
			OS_TCB *current = OSTCBPrioTbl[i];
			TASK_EXTRA_DATA *MyPtr;
			MyPtr=current->OSTCBExtPtr;
			if ((current->OSTCBDly==0)  ){ //means next context switching
				if(i<20||i>27)
				{
					highestPriority = current->OSTCBPrio;//change highest priority
					minDeadline = 2147483647;
				}
				else if(MyPtr->Deadline < minDeadline){ //if current deadline < highest priority
					highestPriority = current->OSTCBPrio;//change highest priority
					minDeadline = MyPtr->Deadline;//change deadline 
                    // sprintf(s, "%10d",i);
                    // PC_DispStr( 0,  15, s, DISP_FGND_BLACK + DISP_BGND_LIGHT_GRAY);
				}
			}
		}
	}
